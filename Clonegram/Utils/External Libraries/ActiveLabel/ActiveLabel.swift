...
@IBDesignable open class ActiveLabel: UILabel {
    ...
    @IBInspectable open var usernameColor: UIColor = .blue {
        didSet { updateTextStorage(parseText: false) }
    }
    @IBInspectable open var usernameSelectedColor: UIColor? {
        didSet { updateTextStorage(parseText: false) }
    }
    @IBInspectable open var moreColor: UIColor = .blue {
        didSet { updateTextStorage(parseText: false) }
    }
    @IBInspectable open var moreSelectedColor: UIColor? {
        didSet { updateTextStorage(parseText: false) }
    }
    ...
    @IBInspectable open var mentionFont: UIFont = UIFont.systemFont(ofSize: 13, weight: .regular) {
        didSet { updateTextStorage(parseText: false) }
    }
    @IBInspectable open var hashtagFont: UIFont = UIFont.systemFont(ofSize: 13, weight: .regular) {
        didSet { updateTextStorage(parseText: false) }
    }
    @IBInspectable open var URLFont: UIFont = UIFont.systemFont(ofSize: 13, weight: .regular) {
        didSet { updateTextStorage(parseText: false) }
    }
    @IBInspectable open var usernameFont: UIFont = UIFont.systemFont(ofSize: 13, weight: .bold) {
        didSet { updateTextStorage(parseText: false) }
    }
    @IBInspectable open var moreFont: UIFont = UIFont.systemFont(ofSize: 13, weight: .regular) {
        didSet { updateTextStorage(parseText: false) }
    }
    ...
    open func handleUsernameTap(_ handler: @escaping (String) -> ()) {
        usernameTapHandler = handler
    }
    
    open func handleMoreTap(_ handler: @escaping (String) -> ()) {
        moreTapHandler = handler
    }
    ...
    func onTouch(_ touch: UITouch) -> Bool {
        ...
        case .ended:
            ...
            case .username(let userHandle): didTapUsername(userHandle)
            case .more(let moreHandle): didTapMore(moreHandle)
    ...
    fileprivate var defaultCustomFont: UIFont = UIFont.systemFont(ofSize: 13)
    
    internal var mentionTapHandler: ((String) -> ())?
    internal var hashtagTapHandler: ((String) -> ())?
    internal var urlTapHandler: ((URL) -> ())?
    internal var usernameTapHandler: ((String) -> ())?
    internal var moreTapHandler: ((String) -> ())?
    ...
    fileprivate func addLinkAttribute(_ mutAttrString: NSMutableAttributedString) {
        ...
        for (type, elements) in activeElements {
            ...
            case .url:
                ...
                attributes[NSAttributedString.Key.font] = URLFont
            case .username:
                attributes[NSAttributedString.Key.foregroundColor] = usernameColor
                attributes[NSAttributedString.Key.font] = usernameFont
            case .more:
                attributes[NSAttributedString.Key.foregroundColor] = moreColor
                attributes[NSAttributedString.Key.font] = moreFont
            case .custom:
                attributes[NSAttributedString.Key.foregroundColor] = customColor[type] ?? defaultCustomColor
                attributes[NSAttributedString.Key.font] = customFont[type] ?? defaultCustomColor
            }
    ...
    fileprivate func updateAttributesWhenSelected(_ isSelected: Bool) {
        ...
        if isSelected {
            ...
            case .username: selectedColor = usernameSelectedColor ?? usernameColor
            case .more: selectedColor = moreSelectedColor ?? moreColor
            case .custom:
            ...
            case .username: unselectedColor = usernameColor
            case .more: unselectedColor = moreColor
    ...
    fileprivate func didTapUsername(_ username: String) {
        guard let usernameHandler = usernameTapHandler else {
            delegate?.didSelect(username, type: .username)
            return
        }
        usernameHandler(username)
    }
    
    fileprivate func didTapMore(_ more: String) {
        guard let moreHandler = moreTapHandler else {
            delegate?.didSelect(more, type: .more)
            return
        }
        moreHandler(more)
    }
